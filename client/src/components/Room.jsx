...
const Room = props => {
    ...
    useEffect(() => {
        ...
        navigator.mediaDevices.getUserMedia({ video: true, audio: true }).then(stream => {
            ...
            socket.emit("[SERVER]-JOIN-ROOM", { roomId, userName: currentUser });
            ...
            socket.on("[CLIENT]-CALL-ACCEPTED", ({ signal, answerId }) => {
                const peerIdx = findPeer(answerId);
                peerIdx.peer.signal(signal);
            });

            socket.on("[CLIENT]-USER-LEFT", ({ userId, userName }) => {
        ...
        socket.on("[SERVER]-TOGGLE-VIDEO-AUDIO", ({ userId, switchTarget }) => {
            const peerIdx = findPeer(userId);
    ...
    function addPeer(incomingSignal, callerId, stream) {
        ...
        peer.on("signal", signal => {
            socket.emit("[SERVER]-ACCEPT-CALL", { signal, to: callerId });
        });
    ...
    const toggleCameraAudio = e => {
        ...
        socket.emit("[SERVER]-TOGGLE-VIDEO-AUDIO", { roomId, switchTarget: target });
    };
